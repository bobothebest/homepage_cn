...
---
  这个博客上的帖子都是用markdown的格式记录的，但是不知道是不是markdown在网页端不兼容，有一部分字体没有办法在网页里面显示出来，我在这个帖子里做个备忘，这样之后希望改格式的时候可以参考。
  由于markdown中字体变化有限，但是支持html格式，所以下面的文字格式变化都是使用html语法写的，但是这样也就要求整篇markdown文件都需要用html书写所以除非对格式很有执念其实直接用md愈语法写会更方便哈哈。
  展示如下，能够显示的就是可以切换的，字体仍为默认黑体的就是这个站点不支持的。
